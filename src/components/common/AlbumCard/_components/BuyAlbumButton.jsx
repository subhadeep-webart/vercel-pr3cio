import { purchaseAlbum } from "@/services/api/packages-api";
import { useRouter } from "next/navigation";

const BuyAlbumButton=({albumId})=>{
    console.log("albumId",albumId)
     const router = useRouter();
        const handelBuyAlbum = (e, albumId) => {
            e.preventDefault();      // prevent default link behavior
            e.stopPropagation();
            console.log("Album Id coming from albums====>", albumId);
    ...
    return(
        <>
         <button
            className="group inline-block mt-4 w-[2rem] h-[2rem] rounded-full inline-flex justify-center items-center bg-[#C6FF00] cursor-pointer" onClick={(e) => handelBuyAlbum(e, albumId)}>
