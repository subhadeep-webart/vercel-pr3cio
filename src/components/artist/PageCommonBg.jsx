    ...
    return (
        <>
            {/* <div
                ...
                className='flex flex-col md:flex-row w-full items-center rounded-[0.75rem] bg-[#1C1919] bg-cover bg-center bg-no-repeat px-10 py-8'>
            ...
            <div
                style={{ backgroundImage: "url('/images/artist/bg.webp')" }}
                className='flex w-full flex-col items-center rounded-[0.75rem] bg-[#1C1919] bg-cover bg-center bg-no-repeat px-4 py-6 md:flex-row md:px-10 md:py-8'>
                <span className='relative mb-6 mr-0 block w-[6rem] flex-shrink-0 rounded-full border border-[#d3d3d3a1] md:mr-6 md:w-[7.5rem]'>
                    <img
                        src={profile?.avatar}
                        alt='artist'
                        className='block h-[6rem] w-[6rem] rounded-full object-cover p-2 md:h-[7.5rem] md:w-[7.5rem]'
                    />
                    {!profile?.is_artist ? (
                        values.avatar ? (
                            <div className='absolute inset-0'>
                ...
                <div className='mb-6 grow text-center md:text-left'>
                    <h3 className='text-base font-semibold md:text-lg'>
                        {profile?.name}
                    </h3>
                    <p className='mb-4 text-sm font-semibold text-[#9D9D9D] md:mb-6 md:text-base'>
                        {profile?.is_artist ? 'Artist' : profile?.email}
                    </p>
                    <div className='flex flex-wrap justify-center gap-3 text-sm md:justify-start md:text-base'>
                    
                        <span
                            className={
                                profile?.is_artist
                                    ? 'inline-block flex items-center gap-1 rounded-[0.75rem] border border-[#d3d3d3a1] px-3 py-1 font-medium'
                                    : 'hidden'
                            }>
            ...
            <div
                className={profile?.is_artist ? 'mb-6 px-4 md:px-0' : 'hidden'}>
                <ul className='relative top-0 float-none inline-flex w-full overflow-x-auto rounded-[0.75rem] bg-[#2A2929] py-2 pr-2 scrollbar-hide md:right-[2rem] md:top-[-1.3rem] md:float-right md:w-auto'>
                    {ARTIST_PROFILE_NAVIGATION.map((nav, index) => {
                        const isActive = pathname.endsWith(nav.path)
                        return (
                            <li key={`nav-item-${index}`}>
                                <Link
                                    href={`/artists${nav.path}?id=${slug}`}
                                    className={
                                        `ml-2 whitespace-nowrap rounded-[0.50rem] px-3 py-1 text-xs font-medium text-white transition-colors duration-300 md:text-sm ` +
                                        (isActive
                                            ? 'bg-gradient-to-r from-[#D344C9] to-[#2D2694]'
                                            : 'bg-[#373437] hover:bg-gradient-to-r hover:from-[#D344C9] hover:to-[#2D2694]')
                                    }>
