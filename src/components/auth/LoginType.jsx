...
export default function LoginToggle({ isArtist, setIsArtist }) {
    // default is User

    return (
        <div className="absolute top-0 z-10 bg-[url('/images/login/shadow.webp')] bg-cover bg-center bg-no-repeat py-4 text-center md:-left-28 md:top-4 lg:-left-28 xl:-left-16 2xl:-left-20">
            {/* User Button */}
            <button
                type='button'
                onClick={() => setIsArtist(false)}
                className={`group mb-2 mt-2 flex h-[2.63rem] w-[7.56rem] items-center justify-center rounded-md border border-[#2E2E2E] px-4 text-base font-semibold transition-colors duration-300 ${
                    isArtist === false
                        ? 'bg-black text-[#7B265D]'
                        : 'bg-[#2E2E2E] text-[#9C9C9C]'
                }`}>
                <svg
                    width='24'
                    height='24'
                    ...
                    className={`mr-1 fill-current text-current transition-colors duration-300 group-hover:text-[#7B265D]`}>
