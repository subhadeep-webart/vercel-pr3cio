    ...
    const { data, hasNextPage, fetchNextPage, status, refetch, isFetchingNextPage } = useInfiniteQuery({
        ...
        queryFn: ({ pageParam = 1 }) =>
            getSongs({
                page: pageParam,
                limit: 10,
                is_trending: false, // Pass the trending prop to the API call
            }),
        initialPageParam: 1,
            ...
            <div className="bg-[#2B2B2B] rounded-[0.75rem] p-6 sm:w-[calc(100%-1rem)] float-right">
                <div className="flex justify-between">
                    ...
                    <ul className="flex">
                        ...
                        <li>
                            <Link className={`bg-[#373437] ml-2 hover:bg-gradient-to-r hover:from-[#D344C9] hover:to-[#2D2694] rounded-[0.50rem] font-medium text-xs text-white px-5 py-2 transition-colors duration-300`} href={"/users/songs/trendings"}>
                                Trending
                            </Link>
                        </li>
                    </ul>
