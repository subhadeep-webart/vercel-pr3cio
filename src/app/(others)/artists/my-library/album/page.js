...
import { getAlbumDetails, getUserAlbums } from "@/services/api/album-ep";
...
const MyAlbumPage = () => {
    ...
    const { data, status } = useQuery({
        queryKey: [queryConstants.getUserAlbums],
    ...
    return (
        <div
            className="grid grid-cols-1 grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 2xl:grid-cols-5 gap-8 min-h-[500px]">
            ...
            {
                albums.map((album) => (<AlbumCard albumDetails={album} key={album?._id} />))
            }
        </div>
    )
}
